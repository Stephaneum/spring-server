<template>
  <div style="margin-top: 60px; display: flex; align-items: center; justify-content: space-evenly">
    ...
    <div class="batch-btn card" @click="showDeleteStorage">
      <i class="material-icons">delete</i>
      <span>Speicher löschen</span>
    </div>
    ...
    <div id="modal-update-password" class="modal" style="width: 500px">
      <div class="modal-content">
        ...
        <div class="input-field">
          ...
          <input v-model="password" type="password" id="input-update-password" autocomplete="off"/>
        </div>
        <br>
        <div class="input-field">
    ...
    <div id="modal-update-quotas" class="modal" style="width: 500px">
      <div class="modal-content">
        ...
        <div class="input-field">
          ...
          <input v-model="storage" type="text" id="input-quotas-storage" autocomplete="off"/>
        ...
        <div class="input-field">
          ...
          <input v-model="grade" type="text" id="input-quotas-grade" autocomplete="off"/>
        </div>
        <br>
      </div>
      <div class="modal-footer">
    ...
    <div id="modal-delete-storage" class="modal" style="width: 500px">
      <div class="modal-content">
        <h4>Nutzerspeicher löschen</h4>
        <br>
        Alle hochgeladenen Dateien einer bestimmten Rolle werden gelöscht.
        <br>
        Bilder in veröffentlichten Beiträgen bleiben erhalten.
        <br><br>
        <div class="input-field">
          <i class="material-icons prefix">vpn_key</i>
          <label for="input-delete-storage-pw">Passwort zur Bestätigung</label>
          <input v-model="password" type="password" id="input-delete-storage-pw" autocomplete="off"/>
        </div>
        <br>
        <div class="input-field">
          <i class="material-icons prefix">person</i>
      ...
      <div class="modal-footer">
        <a href="#!"
           class="modal-close waves-effect waves-green btn-flat">Abbrechen</a>
        <button @click="deleteStorage" type="button" class="btn waves-effect waves-light red darken-3">
    ...
    <div id="modal-delete-users" class="modal" style="width: 500px">
      <div class="modal-content">
        ...
        <div class="input-field">
          ...
          <input v-model="password" type="password" id="input-delete-users-pw" autocomplete="off"/>
        </div>
        <br>
        <div class="input-field">
          <i class="material-icons prefix">person</i>
...
export default {
  ...
  methods: {
    ...
    showDeleteStorage: function () {
      this.password = null;
      this.role = 0;
      M.Modal.getInstance(document.getElementById('modal-delete-storage')).open();
      this.$nextTick(() => {
        M.updateTextFields();
        M.FormSelect.init(document.querySelectorAll('select'), {});
      });
    ...
    deleteStorage: async function () {
      ...
      showLoading('Speicher löschen...')
      try {
        await Axios.post('/api/users/batch/delete-storage', {password: this.password, role: this.role});
        hideLoading();
        M.toast({html: 'Speicher gelöscht.'});
        M.Modal.getInstance(document.getElementById('modal-delete-storage')).close();
      } catch (e) {
